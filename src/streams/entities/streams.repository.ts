...
export class StreamsRepository {
  ...
  findOneOrFail(id: string): Promise<Stream> {
    return this.repo.findOneOrFail({ where: { id }, relations: ['chunks', 'section'] } );
  }

  findOneWithSectionOrFail(id: string): Promise<Stream> {
    return this.repo.findOneOrFail({ where: { id }, relations: ['chunks', 'section', 'section.electionRegion', 'section.town', 'section.cityRegion', 'section.town.municipality', 'section.town.country'] } );
  }
  ...
  findForUser(user: User): Promise<Stream | null> {
    return this.repo.findOne({
      relations: ['section', 'section.electionRegion', 'section.town', 'section.town.municipality', 'section.cityRegion'],
      join: {
        alias: 'stream',
        innerJoin: {
          user: 'stream.user'
