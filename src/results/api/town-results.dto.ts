...
export class TownResultsDto {
  ...
  @Transform(({ value: sections = [], options }: { value: Section[], options: TransformOptions }): Record<string, TownResultsDto> => {
    return sections.reduce((acc: any, section: Section): Record<string, SectionResultsDto> => {
