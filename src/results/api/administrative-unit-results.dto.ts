...
export class AdmUnitResultsDto {
  ...
  @Transform(
    ({
      ...
      value: Record<string, CityRegion>;
      options: ClassTransformOptions;
    }) => {
      Object.entries(districts).forEach(([, district]) => {
        district.towns = (district.towns || []).reduce(
          (acc: any, town: Town) => {
  ...
  public static fromEntity(
    admUnit: Municipality | Country,
    groups: string[] = ['list'],
  ): AdmUnitResultsDto {
    if (Object.keys(admUnit.cityRegions).length === 0) {
      admUnit.cityRegions = { '00': new CityRegion(null, '00', admUnit.towns) };
    }
    const admUnitDto = plainToClass<
      AdmUnitResultsDto,
      Partial<Municipality | Country>
    >(AdmUnitResultsDto, admUnit, {
      groups,
    });
