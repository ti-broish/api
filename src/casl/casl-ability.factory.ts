...
import { Violation, ViolationStatus } from 'src/violations/entities/violation.entity';
...
type Subjects =
  ...
  | typeof Party | Party
  | typeof Violation | Violation
  | 'all';

type Actions = Action.Read | Action.Update | Action.Create | Action.Manage | Action.Delete | Action.Publish;
...
export class CaslAbilityFactory {
  createForUser(user: User) {
    ...
    if (user.hasRole(Role.Admin)) {
      ...
      can([Action.Update, Action.Delete], Broadcast, { publishAt: { $eq: null } });
      can(Action.Update, User, ['email', 'phone', 'firstName', 'lastName', 'pin', 'organization', 'roles']);
    }
    ...
    if (user.hasRole(Role.Validator) || user.hasRole(Role.ExternalValidator) || user.hasRole(Role.Admin)) {
      ...
      if (!user.hasRole(Role.ExternalValidator)) {
        ...
        cannot(Action.Publish, Protocol)
          .because('External validators need another validator to review the protocol before publishing!');
      }
    }
