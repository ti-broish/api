...
export class ViolationCommentsRepository {
  constructor(
    @InjectRepository(ViolationComment)
    private readonly repo: Repository<ViolationComment>,
  ) {}

  findOneOrFail(id: string): Promise<ViolationComment> {
    return this.repo.findOneOrFail({
      where: { id },
      relations: ['violation', 'author'],
    });
  }

  queryBuilderForViolation(
    violation: Violation,
  ): SelectQueryBuilder<ViolationComment> {
